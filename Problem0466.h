...
public:
    int getMaxRepetitions(const string &s1, int n1, const string &s2, int n2) {
        ...
        for (int i = 0, k = 0; i < n1; ++i) {  // i指示第(i+1)个s1，k指示与s2已匹配的字符个数
            ...
            if (prev_idx.count((int) (k % s2.size()))) {  // 存在(counter[x] * s1)曾匹配到相同位置
                auto a = i - prev_idx[(int) (k % s2.size())];  // 这个周期包含a个s1
                auto b = k - counter[prev_idx[(int) (k % s2.size())]];  // 这个周期可匹配b个s2中的字符
